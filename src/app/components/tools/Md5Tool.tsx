...
function md5(str: string): string {
  ...
  function rstr2binl(input: string): number[] {
    ...
    for (let i = 0; i < input.length; i++) output[i >> 2] |= input.charCodeAt(i) << (i % 4 * 8);
    return output;
  }
      ...
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-2 cursor-pointer select-none text-sm text-muted-foreground">
          <div
            ...
            className={`w-10 h-5 rounded-full transition-colors cursor-pointer relative ${uppercase ? "bg-primary" : "bg-muted-foreground/30"}`}
          >
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 p-3 bg-muted rounded-lg border border-border">
          ...
          <button onClick={() => copy(display, "md5")} disabled={!display} className="flex-shrink-0 text-muted-foreground hover:text-primary transition-colors disabled:opacity-40">
            {copied === "md5" ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
          </button>
        </div>
      </div>

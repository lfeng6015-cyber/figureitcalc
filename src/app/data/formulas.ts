...
export const formulaRegistry: Record<string, FormulaConfig> = {
    ...
    formula: (v) => { const P=F(v.principal),pmt=F(v.monthly),r=F(v.rate)/100/Number(v.freq),n=F(v.years)*Number(v.freq); const fv=P*Math.pow(1+r,n)+(r>0?pmt*(Math.pow(1+r,n)-1)/r:pmt*n); const inv=P+pmt*n; var growth=''; for(var y=0;y<=F(v.years);y+=Math.max(1,Math.floor(F(v.years)/6))){var prds=y*Number(v.freq);var v_=P*Math.pow(1+r,prds)+(r>0?pmt*(Math.pow(1+r,prds)-1)/r:pmt*prds);growth+='Yr'+y+': $'+v_.toFixed(0)+' | ';} return [{label:'Future Value',value:'$'+fv.toFixed(2)},{label:'Total Invested',value:'$'+inv.toFixed(0)},{label:'Interest Earned',value:'$'+(fv-inv).toFixed(0)},{label:'Growth Timeline',value:growth.slice(0,-3)},{label:'Effective Rate',value:(Math.pow(1+F(v.rate)/100/Number(v.freq),Number(v.freq))-1).toFixed(4),insight:'APY including compounding. Rule of 72: money doubles in ~'+Math.round(72/F(v.rate)).toFixed(0)+' years at '+F(v.rate).toFixed(0)+'%'}]; },
